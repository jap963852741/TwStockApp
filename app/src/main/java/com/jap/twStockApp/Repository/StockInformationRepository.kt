...
class StockInformationRepository(val updatedataSource: UpdateDataSource) {
    ...
    private fun getStringArray(stockData: TwStock): ArrayList<String> {
        ...
        tempArray.add("--基本面--")
        tempArray.add("殖利率 : " + stockData.DividendYield.toString())
        tempArray.add("本益比 : " + stockData.PriceToEarningRatio.toString())
        tempArray.add("股價淨值比 : " + stockData.PriceBookRatio.toString())
        tempArray.add("--營收成長--")
        tempArray.add("營業收入 : " + stockData.OperatingRevenue.toString())
        tempArray.add("月增率 : " + stockData.MoM.toString())
        tempArray.add("年增率 : " + stockData.YoY.toString())
        tempArray.add("--籌碼--")
        tempArray.add("董監持股比例 : " + stockData.DirectorsSupervisorsRatio.toString())
        tempArray.add("外商持股比例 : " + stockData.ForeignInvestmentRatio.toString())
        tempArray.add("投信持股比例 : " + stockData.InvestmentRation.toString())
        tempArray.add("自營商持股 : " + stockData.SelfEmployedRation.toString())
        tempArray.add("三大法人持股比例 : " + stockData.ThreeBigRation.toString())
        return tempArray
    }
